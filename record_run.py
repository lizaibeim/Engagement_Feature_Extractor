...
CHUNK = 1024
RECORD_SECONDS = 1.51  # read from stream for each 1.5 seconds
    ...
    if state:
        if not shut_down:
            ...
            if compare_images(features_image_path, features_image_path2):
                end_time = time.time()
                print("[INFO] Prediction result for output %d: silent. Time usage: %.5f s\n"%(count, end_time-start_time))
                continue
            ...
            if np.argmax(result) == 1:
                print("[INFO] Prediction result for output %d: normal with prob %s. Time usage: %.5f s\n"%(count, result, end_time-start_time))
                continue
            
            if np.argmax(result) == 2:
                print("[INFO] Prediction result for output %d: overlap with prob %s. Time usage: %.5f s\n"%(count, result, end_time-start_time))
                continue  
